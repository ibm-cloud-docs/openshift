...
|H1510|`Cluster resources low, not enough workers in Ready state.`|The add-on is not ready to be used for one of the following reasons:<ul><li>The cluster does not meet the size criteria for the add-on. </li><li>Worker nodes in your cluster are not in a <code>Normal</code> state. <a href="/docs/containers?topic=containers-debug_worker_nodes">Review the worker nodes' state and status</a>.</li></ul>|
