...
3. Verify that the prerequisite steps for your ALB creation are completed.
    ...
    * ![VPC infrastructure provider icon.](images/icon-vpc-2.png) **VPC clusters**: Verify that the VPC load balancer for your routers exists. In the output, look for the VPC load balancer **Name** that starts with `kube-<cluster_ID>`. If you did not install the `infrastructure-service` plug-in, install it by running `ibmcloud plugin install infrastructure-service`.
    ```
