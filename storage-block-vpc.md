...
Now available, you can choose a [second-generation storage class](/docs/openshift?topic=openshift-storage-block-vpc-sc-ref).
{: tip}
...
1. Create a file for your PVC and name it `pvc.yaml`. Choose between a [first-generation and a second-generation](/docs/openshift?topic=openshift-storage-block-vpc-sc-ref) storage class.
...
* [Storage class reference](/docs/openshift?topic=openshift-storage-block-vpc-sc-ref).  
...
5. Create a configuration file for your PV. Include the **ID**, **Size**, **Zone**, and **IOPS** that you retrieved earlier.
    ...
    `worker_node_primary_IP`
    :   The primary IP of the worker node where you want to attach storage. You can find the primary IP of your worker node by running `ibmcloud oc worker ls`.
...
Before updating the add-on review the [change log](/docs/openshift?topic=openshift-cl-add-ons-vpc-block-csi-driver).
{: tip}
...
1. [Decide if you want to store the {{site.data.keyword.keymanagementserviceshort}} root key CRN in a customized storage class or in a Kubernetes secret](/docs/openshift?topic=openshift-vpc-block#vpc-customize-default). Then, follow the steps to create a customized storage class or a Kubernetes secret.
...
Before you begin
- Review the [Storage class reference](/docs/openshift?topic=openshift-storage-block-vpc-sc-ref) to determine the `profile` that you want to use for your storage class.
...
- You can use an existing storage class as a starting point for creating your own class. Save the details of an existing storage class by using the `oc get sc <storageclass> -o yaml` command.
...
1. Create a customized storage class configuration file in the following format. Choose between a [first-generation and a second-generation](/docs/openshift?topic=openshift-storage-block-vpc-sc-ref) storage class.
