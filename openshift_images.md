...
You can use the internal OpenShift registry to store your container images. The default setup of the registry varies depending on the type of cluster that you choose.
{: shortdesc}

- **Classic clusters**: Your OpenShift cluster is set up by default with an internal registry that uses classic {{site.data.keyword.cloud_notm}} File Storage as the backing storage. When you delete the cluster, the internal registry and its images are also deleted. If you want to persist your images, consider using a private registry such as {{site.data.keyword.registrylong_notm}}, backing up your images to persistent storage such as {{site.data.keyword.objectstorageshort}}, or creating a separate, stand-alone OpenShift container registry (OCR) cluster. For more information, see the [OpenShift docs ![External link icon](../icons/launch-glyph.svg "External link icon")](https://docs.openshift.com/container-platform/3.11/install_config/registry/index.html#install-config-registry-overview).
- **VPC clusters**: The internal registry of your OpenShift cluster is set up to use an `emptyDir` Kubernetes volume that stores your container images on the secondary disk of your worker node. `emptyDir` volumes are not considered persistent highly available storage. You can choose to change the internal registry deployment to [store data to an {{site.data.keyword.cos_full_notm}} bucket](#vpc_setup_cos) instead.


### Classic: Verifying the default setup of the internal registry
...
### VPC: Setting up the internal registry to use {{site.data.keyword.cos_full_notm}}
{: #vpc_setup_cos}

Set up the internal OpenShift registry to use {{site.data.keyword.cos_full_notm}} as the backing storage.
{: shortdesc}

When you create a standard VPC Generation 1 compute OpenShift cluster, the internal OpenShift registry is set up with an `emptyDir` Kubernetes volume by default. Data that is written to an `emptyDir` volume is stored on the worker node's secondary disk. `emptyDir` volumes are not considered highly available persistent storage as data is lost when the worker node is replaced or deleted, or when the internal registry deployment is removed or rescheduled onto another worker node.

You can add persistent storage to the internal registry deployment by setting up an {{site.data.keyword.cos_full_notm}} bucket and mounting this bucket to the registry deployment. {{site.data.keyword.cos_full_notm}} buckets automatically scale in size and you are charged for the cloud storage that you use.

{{site.data.keyword.block_storage_is_short}} (Gen 1 Compute) is not available in OpenShift clusters because the block storage driver requires Kubernetes version 1.15 or higher. Because OpenShift clusters run OpenShift version 3.11, which includes the Kubernetes project version 1.11, you cannot provision block storage as the backing storage for the internal registry.
{: note}

Before you begin, confirm that you have the **Manager** {{site.data.keyword.cloud_notm}} IAM service access role for the [cluster](/docs/openshift?topic=openshift-access_reference#service) and [{{site.data.keyword.cos_full_notm}}](docs/services/cloud-object-storage?topic=cloud-object-storage-iam-bucket-permissions).

1. [Create an instance of {{site.data.keyword.cos_full_notm}}](/docs/openshift?topic=openshift-object_storage#create_cos_service).
2. [Create a secret in your cluster to store the {{site.data.keyword.cos_full_notm}} service credentials](/docs/openshift?topic=openshift-object_storage#create_cos_secret).
3. [Install the {{site.data.keyword.cos_full_notm}} plug-in in your cluster](/docs/openshift?topic=openshift-object_storage#install_cos).
4. [Decide on the configuration of your {{site.data.keyword.cos_full_notm}} bucket](/docs/openshift?topic=openshift-object_storage#configure_cos). You can use one of the [provided storage classes](/docs/openshift?topic=openshift-object_storage#cos_storageclass_reference) or take the following example as the basis to create your own customized storage class.
...
5. Create a persistent volume claim (PVC) from the storage class that you chose. In the following example, you automatically create a bucket in {{site.data.keyword.cos_full_notm}} that is named `<cluster_ID>-registry` and that applies the configuration of the `vpc-registry-storage` storage class that you created earlier.  
   ```
   ...
   metadata:
     ...
     annotations:
       ...
       ibm.io/bucket: "<clusterID>-registry"
   ...
   spec:
     ...
     storageClassName: vpc-registry-storage
   ```
   {: codeblock}

6. Create the PVC in your cluster.
   ```
   oc apply -f pvc.yaml
   ```
   {: pre}

7. Verify that the PVC is created and shows a **Bound** status.
   ```
   oc get pvc
   ```
   {: pre}
   ...
   registry-backing   Bound     pvc-b38b30f9-1234-11e8-ad2b-t910456jbe12   8Gi        RWO            vpc-registry-storage
   ```
   {: screen}

8. Edit the deployment for the internal registry.
   ```
   oc edit deployment docker-registry -n default
   ```
   {: pre}

9. Add the name of the PVC that you created to the `volumes.persistentVolumeClaim.claimName` section and save your changes.
   ```
   volumes:
   - name: registry-storage
     persistentVolumeClaim:
       claimName: registry-backing
   ```
   {: codeblock}

10. Verify that the deployment uses the {{site.data.keyword.cos_full_notm}} PVC that you created.
    ```
    oc describe deployment docker-registry -n default
    ```
    {: pre}
