...
6. Verify your connection.
    ...
    - If the private cloud service endpoint is enabled, you must be in your {{site.data.keyword.cloud_notm}} private network or connect to the private network through a VPN connection to verify your connection to the master. **Note**: You must [expose the master endpoint through a private load balancer](/docs/openshift?topic=openshift-cluster-access-private-vpc) so that users can access the master through a VPN or {{site.data.keyword.BluDirectLink}} connection.
...
To maintain connectivity for your worker nodes to {{site.data.keyword.openshiftlong_notm}}, complete the following steps before 31 July 2026.
...
- **If you have custom scripts that call origin endpoints directly**: Switch to the supported API endpoints. For more information, see [About the API](/docs/openshift?topic=openshift-cs_api_install#api_about).
...
Enable worker-to-worker communication by allowing all TCP, UDP, VRRP, and IPEncap traffic between worker nodes on the private interfaces and also allow VRRP on the public interface. {{site.data.keyword.openshiftlong_notm}} uses the VRRP protocol to manage IP addresses for load balancers and the IPEncap protocol to permit pod to pod traffic across subnets.
