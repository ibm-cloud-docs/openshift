...
3. Configure disaster recovery for your cluster. 
    **Metro DR**:
    ...
    3. Set up a [Databases for etcd service instance](/docs/openshift?topic=openshift-storage-portworx-kv-store) for your Portworx key-value store. Because both {{site.data.keyword.redhat_openshift_notm}} clusters must share the key-value store, you can't use the internal Portworx KVDB.
    ...
    5. Follow the instructions to [install Portworx](/docs/openshift?topic=openshift-storage-install-portworx) with the disaster recovery plan in both of your clusters. If you installed Portworx without the disaster recovery plan in one of your clusters already, you must re-install Portworx in that cluster with the disaster recovery plan. Make sure that you select **Databases for etcd** from the **Portworx metadata key-value store** drop-down and that you enter the same Databases for etcd API endpoint and Kubernetes secret name in both of your clusters.
    ...
    **Asynchronous DR**:
    ...
    5. Follow the instructions to [install Portworx](/docs/openshift?topic=openshift-storage-install-portworx) with the disaster recovery plan in both of your clusters. If you installed Portworx without the disaster recovery plan in one of your clusters already, you must re-install Portworx in that cluster with the disaster recovery plan. Make sure that you configure the Portworx key-value store that each cluster uses.
