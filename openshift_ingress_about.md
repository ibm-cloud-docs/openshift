...
<ff-roks311-vpc>Depending on whether you have a classic or VPC cluster, a</ff-roks311-vpc>A Cloudflare multizone load balancer (MZLB)<ff-roks311-vpc> or a Load Balancer for VPC</ff-roks311-vpc> health checks your ALBs.
...
6. When the app returns a response packet, it uses the IP address of the worker node where the ALB that forwarded the client request exists. The ALB then sends the response packet to the client.<ff-roks311-vpc>
