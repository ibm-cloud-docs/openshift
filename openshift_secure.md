...
<caption>Worker node security setup</caption>
  ...
  <tbody>
    <tr><td>CIS-compliant RHEL image</td><td>Every worker node is set up with an Red Hat Enterprise Linux (RHEL) operating system that implements the benchmarks that are published by the Center of Internet Security (CIS). The operating system cannot be changed by the user or the owner of the machine. To review the current RHEL version, run <code>{[kubectl]} get nodes -o wide</code>. IBM works with internal and external security advisory teams to address potential security compliance vulnerabilities. Security updates and patches for the operating system are made available through {[product_name_notm]} and must be installed by the user to keep the worker node secure.<p class="important">{[product_name_notm]} uses a Red Hat Enterprise Linux kernel for worker nodes. You can run containers based on any Linux distribution in {[product_name_notm]}. Check with your container image vendor to verify that your container images can be run on an Red Hat Enterprise kernel.</p></td></tr>
    <tr>
...
**Can I use the built-in container registry in my {{site.data.keyword.openshiftshort}} cluster to address build automation, image and container scanning?** </br>
You can use the built-in container registry to automate the container image build process from your source code in an external source repository to your internal registry. However, images are not automatically scanned for vulnerabilities when they are pushed to the internal registry. To set up image scanning, set up a registry namespace and push your images to the managed {{site.data.keyword.registryshort_notm}} instead. 
...
**What is an {{site.data.keyword.openshiftshort}} project and why should I use it?** </br>
...
<dl>
  ...
  <dd>Store personal information only in Kubernetes resources that are designed to hold personal information. For example, do not use your name in the name of an {{site.data.keyword.openshiftshort}} project, deployment, service, or config map. For proper protection and encryption, store personal information in <a href="https://docs.openshift.com/container-platform/3.7/dev_guide/secrets.html">secrets</a> instead.</dd>
