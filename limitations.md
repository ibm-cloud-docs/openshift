...
| {{site.data.keyword.redhat_openshift_notm}} | Make sure to review the [OpenShift Container Platform limitations](https://docs.openshift.com/container-platform/4.17/scalability_and_performance/planning-your-environment-according-to-object-maximums.html){: external} for your version.|
...
| Worker node quota | A maximum 500 worker nodes per cluster for any accounts created before 01 January 2024. For accounts created on or after that date, the maximum quota is 200 worker nodes per cluster after a period of lower quotas. Quotas apply per cluster [infrastructure provider](/docs/openshift?topic=openshift-overview#what-compute-infra-is-offered). If you need more of the resource, [contact IBM Support](/docs/account?topic=account-using-avatar). In the support case, include the new quota limit for the region and infrastructure provider that you want.. To list quotas run, `ibmcloud ks quota ls`. |
