...
7. For workloads that need to scale up and down in response to resource requests, set up the [horizontal pod autoscaler](/docs/openshift?topic=openshift-update_app#app_scaling) and [cluster worker pool autoscaler](/docs/openshift?topic=openshift-cluster-scaling-install-addon).
...
[Apply labels to worker nodes](/docs/openshift?topic=openshift-worker-tag-label) with labels or update an existing worker pool.
