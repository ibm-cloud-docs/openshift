...
1. If restarting your pod does not resolve the issue, [reload your worker node](/docs/openshift?topic=openshift-kubernetes-service-cli#worker-reload-cli).
...
1. Verify that the `kubectl` CLI version that you run on your local machine matches the Kubernetes version that is installed in your cluster. If you use a `kubectl` CLI version that does not match at least the major.minor version of your cluster, you might experience unexpected results. For example, [Kubernetes does not support `kubectl` client versions that are 2 or more versions apart from the server version (n +/- 2).
    ...
    2. If the `kubectl` CLI versions on your local machine and your cluster don't match, either [update your cluster](/docs/openshift?topic=openshift-update) or [install a different CLI version on your local machine](/docs/openshift?topic=openshift-cli-install).
...
1. For classic block storage, object storage, and Portworx only: Make sure that you installed the latest Helm chart version for the plug-in.
    ...
    **Block and object storage**:
    ...
    4. If a more recent version is available, install this version. For instructions, see [Updating the {{site.data.keyword.cloud_notm}} Block Storage plug-in](/docs/openshift?topic=openshift-vpc-block#vpc-addon-update) and [Updating the {{site.data.keyword.cos_full_notm}} plug-in](/docs/openshift?topic=openshift-storage_cos_install#update_cos_plugin).
...
3. If a more recent version is available, install this version. For instructions, see [Updating Portworx in your cluster](/docs/openshift?topic=openshift-storage_portworx_update).
