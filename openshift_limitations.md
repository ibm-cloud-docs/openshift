...
| GPU bare metal worker nodes | Because GPU is not generally available from the community out of the box, GPU bare metal worker nodes are not available. You can still order [bare metal worker nodes](/docs/openshift?topic=openshift-planning_worker_nodes#bm-table) that are tailored to balanced, data-intensive, and RAM-intensive workloads.|
...
| Router health checks | If you set up [VPC security groups](/docs/openshift?topic=openshift-vpc-network-policy#security_groups) or [VPC access control lists (ACLs)](/docs/openshift?topic=openshift-vpc-network-policy#acls) to secure your cluster network, inbound access is blocked from the {{site.data.keyword.openshiftshort}} control plane and Cloudflare's IPv4 IPs that are used to health check the IP addresses of your router services. Because each ACL has a quota of only 50 rules and each security group has a quota of only 25 rules, you currently cannot add enough inbound and outbound rules for each {{site.data.keyword.openshiftshort}} control plane and Cloudflare IP address. Due to this known limitation your Ingress status shows that your routers are unreachable; however, this status does not reflect your router services' actual health. |
