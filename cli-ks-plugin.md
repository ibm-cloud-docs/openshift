...
#### `cluster addon disable image-key-synchronizer` command options
...
#### `cluster addon disable openshift-data-foundation` command options
...
#### `cluster addon enable image-key-synchronizer` command options
...
#### `cluster addon enable openshift-data-foundation` command options
...
:    Optional: Specify the type of endpoint to use to connect to the cluster. If you don't specify this option, the default service endpoint for your cluster is used.
     ...
     - `link`: To connect to {{site.data.keyword.satellitelong_notm}} clusters from within the {{site.data.keyword.cloud_notm}} private network, set to `link` to use your {{site.data.keyword.satelliteshort}} location's Link endpoint for the cluster context. If you specify this option, you must also specify the `--admin` option. If you are not connected to the {{site.data.keyword.cloud_notm}} private network, this option is not required because the cluster service URL is used.
        Before you can use the `link` endpoint type, make sure that you set up the required **Source** [in your Location](/docs/satellite?topic=satellite-link-endpoint-secure).
        {: note}
...
:    Set this option to `cloud_pak` only if you use this cluster with a Cloud Pak that has an OpenShift entitlement. When you specify the number of workers (`--workers`) and flavor (`--flavor`), make sure to specify only the number and size of worker nodes that you are entitled to use in [IBM Passport Advantage](https://www.ibm.com/software/passportadvantage){: external}. After your cluster is created, you are not charged the {{site.data.keyword.redhat_openshift_notm}} license fee for the entitled worker nodes in the `default` worker pool.
...
```sh
ibmcloud oc cluster create vpc-gen2 --name mycluster --version 4.21_openshift --zone us-south-1 --vpc-id a0123456-78b9-0c1d-23d4-567890123ef4 --subnet-id 1ab23c45-6789-0123-456d-789ef01gh234 --flavor bx2.4x16 --workers 3
```
{: pre}
...
```sh
ibmcloud oc worker reload --worker WORKER_ID [--worker WORKER_ID ...] [--cluster CLUSTER] [--skip-master-healthcheck] [-f] [-q]
```
{: pre}
...
`--entitlement ocp_entitled`
:    Include this option only for a cluster that has a {{site.data.keyword.redhat_openshift_notm}} entitlement. When you specify the number of workers (`--size-per-zone`) and flavor (`--flavor`), make sure to specify only the number and size of worker nodes that you are entitled to use in [IBM Passport Advantage](https://www.ibm.com/software/passportadvantage){: external}. After creation, your worker pool does not charge you the {{site.data.keyword.redhat_openshift_notm}} license fee for your entitled worker nodes.
...
```sh
ibmcloud oc zone add classic --zone dal10 --cluster my_cluster -p pool1 -p pool2 --private-vlan 2294021
```
{: pre}
...
### `ibmcloud oc ingress lb backend set`
...
```sh
ibmcloud oc ingress lb backend set --cluster CLUSTER [--private-backend BACKEND] [--public-backend BACKEND] [-q]
```
{: pre}
...
```sh
ibmcloud oc ingress lb backend set --cluster mycluster --public-backend traefik
```
{: pre}
...
```sh
ibmcloud oc ingress lb proxy-protocol enable --cluster CLUSTER [--cidr CIDR ...] [-f] [-q]
```
{: pre}
...
```sh
ibmcloud oc nlb-dns monitor configure --cluster mycluster --nlb-host mycluster-a1b2cdef345678g9hi012j3kl4567890-0001.us-south.containers.appdomain.cloud --enable --type HTTPS --path / --timeout 5 --interval 60 --header Host=example.com
```
{: pre}
...
List the locations that are supported by {{site.data.keyword.containerlong_notm}}. For more information about the locations that are returned, see [{{site.data.keyword.containerlong_notm}} locations](/docs/openshift?topic=openshift-regions-and-zones).
{: shortdesc}
...
#### `cluster create satellite` command options
...
```sh
ibmcloud sat cluster create satellite --name mysatcluster --location my-location --pull-secret <secret> --operating-system REDHAT_8_64 --version 4.21_openshift -hl cpu=4 -hl memory=16265432 --workers 3 --zone myzone1
```
{: pre}
...
#### `cluster master satellite-service-endpoint allowlist add` command options
...
#### `cluster master satellite-service-endpoint allowlist disable` command options
...
#### `cluster master satellite-service-endpoint allowlist enable` command options
...
#### `cluster master satellite-service-endpoint allowlist get` command options
...
#### `cluster master satellite-service-endpoint allowlist remove` command options
...
#### `worker-pool create satellite` command options
...
#### `zone add satellite` command options
