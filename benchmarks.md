...
| 1.2.13 | Ensure that the admission control plug-in `SecurityContextDeny` is set if `PodSecurityPolicy` is not used | Not Scored | 1 | [Pass](#ibm-remediations-and-explanations) | {{site.data.keyword.IBM_notm}} |
...
| 1.2.16 | Ensure that the admission control plug-in `PodSecurityPolicy` is set | Scored | 1 | [Pass](#ibm-remediations-and-explanations) | {{site.data.keyword.IBM_notm}} |
...
| 5.6.3 | {{site.data.keyword.openshiftlong_notm}} deploys some system components that don't set a [pod or container `securityContext`](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/). |
