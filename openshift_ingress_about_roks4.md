...
An HAProxy-based OpenShift router is created by the Ingress operator for each Ingress controller. This router is configured with the same domain that is specified in the Ingress controller and listens for incoming HTTP, HTTPS, or TCP service requests through that domain. The router's load balancer component then forwards requests to the pods for that app only according to the rules defined in the Ingress resource and implemented by the Ingress controller. One router per Ingress controller is created in each zone of your cluster.
{: shortdesc}

To find the IP address of the router for the default Ingress controller, run `oc get svc router-default -n openshift-ingress` and look for the router's **EXTERNAL IP** field.
