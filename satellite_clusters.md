...
1. Make sure that you have either the **Administrator** or the **Satellite cluster creator** access role in IAM. For more information, see [IAM platform and service access roles](/docs/satellite?topic=satellite-iam-platform-access).
...
:   Optional. The operating system of the worker nodes in your cluster. For a list of available operating systems by cluster version, see the [{{site.data.keyword.openshiftshort}} version information](/docs/openshift?topic=openshift-openshift_versions). If no option is specified, the default operating system that corresponds to the cluster version is used.
    ...
    ```sh
    ibmcloud oc cluster create satellite --location LOCATION --name NAME --pull-secret SECRET --version 4.20_openshift [--enable-admin-agent] [--host-label LABEL ...] [--operating-system SYSTEM] [--pod-subnet SUBNET] [-q] [--service-subnet SUBNET] [--workers WORKERS-PER-ZONE] [--zone ZONE] [--entitlement ENTITLEMENT]
    ```
    {: pre}
