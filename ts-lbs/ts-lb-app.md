...
1. For version 2.0 NLBs: Ensure that you complete the [NLB 2.0 prerequisites](/docs/openshift?topic=openshift-loadbalancer-v2#ipvs_provision).
...
1.  Check your NLB service and review the **Events** section to find potential errors.  
    ...
    1. Make sure that the public and private VLAN IDs for the worker nodes that were returned by the `oc get nodes` and the `ibmcloud oc worker get` commands match.
