...
Have scripts that you want to update to use the beta formatting? Check out the [`ibmcloud oc script update` command](#script_update).
{: tip}
...
<col width="10%">
 ...
 <tbody>
  ...
  <tr><td>Positional arguments<ul><li>Legacy: Arguments specified by position (`ibmcloud oc cluster-get mycluster`)</li><li>Beta: Arguments specified by flags (`ibmcloud oc cluster get --cluster mycluster`)</li></ul></td>
  <td>Legacy and beta</td>
  <td>Legacy and beta</td>
  <td>Legacy and beta</td>
  <td>Beta</td>
  </tr>
  <tr><td>Repeated arguments<ul><li>Legacy: Comma-delineated values (`ibmcloud oc zone-add-classic --worker-pools pool1,pool2,pool3 ...`)</li><li>Beta: Repeated flags for each value with optional shorthand flag aliases (`ibmcloud oc zone add classic -p pool1 -p pool2 ...`)</li></ul></td>
  <td>Legacy</td>
  <td>Legacy</td>
  <td>Legacy and beta</td>
  <td>Beta</td>
  ...
  <tr><td>Cluster context<ul><li>Legacy: `ibmcloud oc cluster-config` provides a command that you must copy and paste to set the new `kubeconfig` file as your current KUBECONFIG environment variable. You must set your environment variable before you can interact with your cluster.</li><li>Beta: `ibmcloud oc cluster config` appends the new `kubeconfig` file to your existing `kubeconfig` in `~/.kube/config` or the first file in the KUBECONFIG environment variable. After you run `ibmcloud oc cluster config`, you can interact with your cluster immediately.</li></ul></td>
  <td>Legacy</td>
  <td>Legacy</td>
  <td>Legacy</td>
  <td>Beta</td>
...
Update worker nodes to apply the latest security updates and patches to the operating system, and to update the Kubernetes version to match the version of the Kubernetes master. You can update the master Kubernetes version with the `ibmcloud oc cluster master update` [command](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_cluster_update). The worker node IP address remains the same after the update operation.
{: shortdesc}
...
```
ibmcloud oc key-protect-enable --cluster CLUSTER_NAME_OR_ID --key-protect-url ENDPOINT --key-protect-instance INSTANCE_GUID --crk ROOT_KEY_ID [-s]
```
{: pre}
...
<dd>The host name that you want to add IPs to. To see existing host names, run <code>ibmcloud oc nlb-dns ls</code>.</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>The host name that you want to remove an IP from. To see existing host names, run <code>ibmcloud oc nlb-dns ls</code>.</dd>
...
<dd>The host name to configure a health check monitor for. To list host names, run <code>ibmcloud oc nlb-dns ls --cluster CLUSTER</code>.</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>The host name that the monitor health checks. To list host names, run <code>ibmcloud oc nlb-dns ls --cluster CLUSTER</code>.</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>Include this flag to view the status for only one host name. To list host names, run <code>ibmcloud oc nlb-dns ls --cluster CLUSTER</code>.</dd>
...
**Example**:
```
ibmcloud oc script update my_file
```
{: pre}
