...
| 13 July 2020 | The minimum number of worker nodes is changed from 2 worker nodes per zone to 2 worker nodes per cluster, so that default OpenShift components can be deployed. The minimum number of 1 worker node per worker pool is unchanged. |
