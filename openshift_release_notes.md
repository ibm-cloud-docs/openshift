...
| 28 July 2020 | <ul><li>**Ingress ALB versions**: Added the `ibmcloud oc alb versions` CLI command to show the available versions for the ALBs in your cluster and the `ibmcloud oc alb update` CLI command to update or rollback ALBs to a specific version.</li><li>**UI for creating clusters**: Updated getting started and task topics for the updated process for the [{{site.data.keyword.openshiftshort}} cluster creation console](https://cloud.ibm.com/kubernetes/catalog/create?platformType=openshift){: external}.</li></ul> |
...
| 29 April 2020 | <ul><li>**ALB pod scaling**: Added steps for sclaing up your ALB processing capabilities by [increasing the number of ALB pods replicas](/docs/openshift?topic=openshift-ingress-manage#alb_replicas).</li></ul> |
