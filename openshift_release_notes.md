...
| Date | Description |
| ---- | ----------- |
| 13 January 2021 | **Cluster autoscaler**: Cluster autoscaler add-on patch update `1.0.1_210` is released. For more information, see the [Cluster autoscaler add-on changelog](/docs/openshift?topic=openshift-ca_changelog) |
| 12 January 2021 | <ul><li>**Kubernetes benchmarks**: Added how to [run the CIS Kubernetes benchmark tests on your own worker nodes](/docs/containers?topic=containers-cis-benchmark#cis-worker-test).</li><li>**Removal of data center support**: Update the documentation to reflect that Melbourne (`mel01`) is no longer available as an option to create {{site.data.keyword.cloud_notm}} resources in. For more information, see [Withdrawal of support for some data centers](/docs/get-support?topic=get-support-dc-migrate).</li></ul>|
