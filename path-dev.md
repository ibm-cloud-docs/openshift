...
- [Deploying apps with the Kubernetes dashboard](/docs/openshift?topic=openshift-deploy_app).
- [Deploying apps with the CLI](/docs/openshift?topic=openshift-deploy_app).
...
4. **{{site.data.keyword.logs_full_notm}} and {{site.data.keyword.mon_short}}**: To monitor cluster health, forward logs to [{{site.data.keyword.logs_full_notm}}](/docs/openshift?topic=openshift-health#openshift_logging) and metrics to [{{site.data.keyword.mon_full_notm}}](/docs/openshift?topic=openshift-health-monitor).
...
2. **Secret encryption**: [Set up a KMS provider](/docs/openshift?topic=openshift-encryption-setup) and [Encrypt secrets and verify encryption](/docs/openshift?topic=openshift-encryption-secrets).
...
2. **Expose your app**:
    - Load balancers:
        ...
        - VPC clusters: Set up a [VPC load balancer](/docs/openshift?topic=openshift-vpclb-about).
