...
## How do I create an {{site.data.keyword.openshiftlong_notm}} cluster?
{: #faq-cluster-create}
{: faq}
{: support}
...
:   Begin by reviewing the [Getting started](/docs/openshift?topic=openshift-getting-started)  doc, then [create your cluster environment strategy](/docs/openshift?topic=openshift-strategy).
...
For more information about how your cluster resources are set up, see the [Service architecture](/docs/containers?topic=containers-service-arch). To find a list of capabilities and benefits, see [Benefits and service offerings](/docs/openshift?topic=openshift-overview).
...
Periodically, {{site.data.keyword.redhat_openshift_notm}} releases [major, minor, or patch updates](/docs/containers?topic=containers-cs_versions#update_types). These updates can affect the {{site.data.keyword.redhat_openshift_notm}} API server version or other components in your {{site.data.keyword.redhat_openshift_notm}} master. IBM automatically updates the patch version, but you must update the master major and minor versions. For more information, see [Updating the master](/docs/openshift?topic=openshift-managed-addons#updating-managed-add-ons).
...
For examples of the types of workloads that users typically move to the various types of clouds, see [Moving your workloads to {{site.data.keyword.cloud_notm}}](/docs/openshift?topic=openshift-overview). You might also choose a hybrid approach where you have clusters that run in both environments.
{: shortdesc}
...
{{site.data.keyword.openshiftlong_notm}} uses {{site.data.keyword.iamshort}} (IAM) to grant access to cluster resources through IAM platform access roles and Kubernetes role-based access control (RBAC) policies through IAM service access roles. For more information about types of access policies, see [Pick the correct access policy and role for your users](/docs/openshift?topic=openshift-iam-platform-access-roles).
...
For more information about available bare metal flavors and how bare metal is different from virtual machines, see the [planning guidance](/docs/openshift?topic=openshift-strategy#env_flavors_node).

## What is the minimum size cluster that I can make?
{: #smallest_cluster}
{: faq}

Note that running a minimal cluster does not meet the service level agreement (SLA) to receive support. Also, keep in mind that some services, such as Ingress, require highly available worker node setups. You might not be able to run these services or your apps in clusters with only two nodes in a worker pool. For more information, see the [Planning your cluster for high availability](/docs/openshift?topic=openshift-strategy).
{: important}

Classic or VPC clusters
:   Clusters must always have at least 2 worker nodes. Note that you can't have a cluster with 0 worker nodes, and you can't power off or suspend billing for your worker nodes.
...
For a list of supported worker node operated systems by cluster version, see the [{{site.data.keyword.openshiftshort}} version information](/docs/openshift?topic=openshift-openshift_versions).
...
- VPC clusters can be set up only in multizone regions, and as such can always be converted from a single zone cluster to a multizone cluster. For more information, see [Adding worker nodes to VPC clusters](/docs/openshift?topic=openshift-add-workers-vpc).
- Classic clusters that are set up in data centers with only one zone can't be converted to a multizone cluster. For more information, see [Adding worker nodes to Classic clusters](/docs/openshift?topic=openshift-add-workers-classic).
...
For more information about how to achieve high availability for your cluster, see [High availability for {{site.data.keyword.openshiftlong_notm}}](/docs/openshift?topic=openshift-strategy).
...
To view detailed system requirements, you can run a [software product compatibility report for {{site.data.keyword.openshiftlong_notm}}](https://www.ibm.com/software/reports/compatibility/clarity/index.html){: external}. Note that compliance depends on the underlying [infrastructure provider](/docs/openshift?topic=openshift-overview#what-compute-infra-is-offered) for the cluster worker nodes, networking, and storage resources.
...
**{{site.data.keyword.satelliteshort}}**: See the [{{site.data.keyword.satellitelong_notm}} documentation](/docs/satellite?topic=satellite-faqs#standards).
...
You can create the cluster or the worker pool within an existing cluster with the Cloud Pak entitlement in the console or by using the `--entitlement ocp_entitled` option in the [`ibmcloud oc cluster create classic`](/docs/openshift?topic=openshift-kubernetes-service-cli#cluster-create-classic-cli) or [`ibmcloud oc worker-pool create classic`](/docs/openshift?topic=openshift-kubernetes-service-cli#worker-pool-create-classic-cli) CLI commands. Make sure to specify the correct number and flavor of worker nodes that you are entitled to use.
...
## How much do confidential containers cost?
{: #conf-cont-cost}
...
For production, use the Intel Trust Authority and configure INITDATA to use the Intel Trust Authority. You must permit your cluster to communicate with Intel, such as security groups, secure by default OpenShift permissions, and so on.
