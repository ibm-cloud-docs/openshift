...
Looking for general information about updating clusters, or information on a different version? See [Red Hat {{site.data.keyword.redhat_openshift_notm}} on IBM Cloud version information](/docs/openshift?topic=openshift-openshift_versions) and the version [4.14 blog](https://www.redhat.com/en/blog/red-hat-openshift-414-now-available){: external}.
{: tip}
...
| Upgrade requires resolution to OpenShift cluster version upgradeable conditions | A cluster master upgrade will now be cancelled if the OpenShift cluster version `Upgradeable` status condition indicates that the cluster is not upgradeable. Run `oc get clusterversion version -o json \| jq '.status.conditions[] \| select(.type == "Upgradeable")'` to determine if the cluster is upgradeable. If not, the condition information will provide instructions that must be followed before upgrading. |
