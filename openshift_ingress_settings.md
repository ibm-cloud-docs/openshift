...
### Adding {{site.data.keyword.appid_short_notm}} for ALBs that run the Red Hat OpenShift on IBM Cloud Ingress image
{: #app-id-iks}

If your ALB runs the [Red Hat OpenShift on IBM Cloud custom Ingress image](/docs/openshift?topic=openshift-ingress-types), you can use the custom Red Hat OpenShift on IBM Cloud annotation, `ingress.bluemix.net/appid-auth`, in your Ingress resources. For steps on how to set up Ingress with {{site.data.keyword.appid_short_notm}} and how to configure the annotation, see [{{site.data.keyword.appid_short_notm}} Authentication (`appid-auth`)](/docs/openshift?topic=openshift-ingress_annotation#appid-auth).
{: shortdesc}

### Adding {{site.data.keyword.appid_short_notm}} for ALBs that run the community Kubernetes Ingress image
{: #app-id-kube}

If your ALB runs the [community Kubernetes Ingress image](/docs/openshift?topic=openshift-ingress-types), you can use the following steps to configure Ingress with {{site.data.keyword.appid_short_notm}}.
{: shortdesc}
...
4. Get the contents of the secret that was created in your cluster namespace. In the output, note the values of the `oauthServerURL`, `clientId`, and `secret` fields.
  ```
  oc get secret binding-<App_ID_service_instance_name> -n <project> -o=jsonpath='{$.data.binding}' | base64 -D
  ```
  {: pre}
...
7. Use the configuration file to create a Kubernetes secret.
  ```
  oc create secret generic <App_ID_service_instance_name> --from-file=./<App_ID_service_instance_name>.conf -n <project>
  ```
  {: pre}
...
11. Apply the OAuth2-Proxy Ingress resource to your cluster.
  ```
  oc create -f oauth2_proxy_ingress.yaml
  ```
  {: pre}
...
13. Re-apply your Ingress resources to enforce {{site.data.keyword.appid_short_notm}} authentication.
  ```
  oc create -f <app_ingress_resource>.yaml
  ```
  {: pre}
