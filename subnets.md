...
VLAN spanning
:   If you can't or don't want to enable VRF, [enable VLAN spanning](/docs/vlans?topic=vlans-vlan-spanning#vlan-spanning). To perform this action, you need the **Network > Manage Network VLAN Spanning** [infrastructure permission](/docs/openshift?topic=openshift-classic-roles), or you can request the account owner to enable it. To check if VLAN spanning is already enabled, use the `ibmcloud oc vlan spanning get --region <region>` [command](/docs/openshift?topic=openshift-kubernetes-service-cli#cs_vlan_spanning_get). Note that you can't enable the private cloud service endpoint if you choose to enable VLAN spanning instead of a VRF.
...
2. [Create a cluster from the CLI](/docs/openshift?topic=openshift-cluster-create-classic) by using the VLAN ID that you identified. Include the `--no-subnet` option to prevent a new portable public IP subnet and a new portable private IP subnet from being created automatically.
