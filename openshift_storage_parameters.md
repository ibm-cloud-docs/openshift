...
| `numOfOsd` | Specify the number of Object Storage Daemons (OSDs) to create. Starting in OpenShift Data Foundation 4.21, `flexibleScaling` is automatically enabled for single-zone clusters and deployments with fewer than three availability zones, allowing granular scaling instead of requiring multiples of three. For multizone clusters with three or more zones, OpenShift Data Foundation creates three times the specified value to distribute OSDs across zones for high availability. For example, specifying `1` creates 3 OSDs. For single-zone clusters with flexible scaling enabled, the specified value is used directly. For example, specifying `1` creates 1 OSD. For initial deployment in single-zone clusters, specifying at least 3 OSDs is recommended to ensure adequate resiliency. | `1` | Yes |
