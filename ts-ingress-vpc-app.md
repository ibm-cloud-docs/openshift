...
4.  If the load balancer exists, [view the VPC security groups that are attached to it](/docs/openshift?topic=openshift-vpc-security-group). Ensure that you have not made any modifications to the `kube-<vpc-id>` security group, which is automatically applied to the VPC ALB when it is created. 
