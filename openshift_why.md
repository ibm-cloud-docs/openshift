...
| Storage | You must set up persistent volumes to be backed up by a storage provider. OpenShift Container Storage (OCS) is available. | IBM automatically sets up storage providers such as {{site.data.keyword.cloud_notm}} File and Block. OpenShift Container Storage is not available. For supported storage options, see [Planning highly available persistent storage](/docs/openshift?topic=openshift-storage_planning). |
