...
The worker nodes might be overloaded when they frequently report back a status of `NotReady` or evict pods due to lack of memory or other resources. Consider re-evaluating the [capacity requirements for the worker node](/docs/openshift?topic=openshift-strategy#env_resources_worker_capacity).
