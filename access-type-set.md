...
:   This is default access type for version 4.13. For more information, see [Accessing VPC clusters through the VPE gateway](/docs/openshift?topic=openshift-cluster-access-vpe).

Private Service Endpoint URL (`legacy`)
:   This is default access type for cluster versions 4.12 and earlier and 4.14 and later. For more information, see [Accessing clusters through the private cloud service endpoint](/docs/openshift?topic=openshift-cluster-access-private-vpc).
...
## Setting the OAuth access type
{: #oauth-access-set}

Use one of the following methods to set the OAuth access type for your cluster.
...
1. [Generate an IAM token](/docs/iam?topic=iam-iamtoken_from_apikey&interface=ui).
...
1. After setting the access type, perform a [cluster master refresh](/docs/openshift?topic=openshift-kubernetes-service-cli#cluster-master-refresh-cli).
    ```sh
    ibmcloud oc cluster master refresh --cluster CLUSTER
    ```
    {: pre}

## Getting the OAuth access type
{: #oauth-access-get}

Use one of the following methods to retrieve the current OAuth access type for your cluster.

### From the CLI
{: #oauth-access-get-cli}
{: cli}

1. Run the `cluster master console-oauth-access get` command.
