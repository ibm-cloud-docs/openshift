...
- **Pre-configured storage**: Choose between IBM Cloud File Storage or OpenShift Data Foundation (ODF) during cluster creation. Your selected storage solution is automatically deployed and configured.
...
| **Minimum version** | OpenShift 4.20 | OpenShift 4.17 |
...
- **Storage**: IBM Cloud File Storage or OpenShift Data Foundation (ODF) - you choose during cluster creation
...
To identify flavors that support Virtualization Service:
```sh
ibmcloud ks flavors --zone <zone> --provider vpc-gen2 | grep openshift-vs
```
{: pre}
...
Pricing includes:
- Infrastructure costs (bare metal worker nodes)
...
- Storage costs (if using IBM Cloud File Storage, charges apply separately; if using ODF, NVME storage cost is included in the infrastructure cost)
